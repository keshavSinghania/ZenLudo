...
const OtpVerification = () => {
    ...
    const sendOtp = useCallback(async () => {
        if (!email) {
            setResMessage("Error: Email is missing. Please go back and try again.");
            return;
        }
        setResMessage("Sending OTP...");
        try {
    ...
    return (
        <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-purple-900 via-blue-900 to-pink-900 text-white flex items-center justify-center p-4">
            ...
            <div className="relative z-10 p-8 rounded-3xl bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-lg shadow-2xl w-full max-w-sm border-2 border-purple-500 border-opacity-30">
                ...
                <p className="text-center text-sm mb-2 text-gray-300">
                    We've sent a 6-digit OTP to your email:
                    <br />
                    <span className="font-bold text-lg text-white break-words">{email}</span>
                </p>
                ...
                <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                    ...
                    <div className="flex flex-col items-center space-y-3">
                        <button
                            ...
                            className={`w-full py-3 rounded-full text-lg font-semibold transition duration-300 ease-in-out transform hover:scale-105 shadow-lg ${otp.join("").length === OTP_LENGTH
                        ...
                        <p className={`text-sm text-center font-semibold ${resMessage.includes("Error") || resMessage.includes("Failed") ? "text-red-400" : "text-green-400"}`}>
                            {resMessage}
                        </p>

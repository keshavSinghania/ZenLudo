...
function Chat() {
  ...
  const fetchConversation = async (friendId, reset = false) => {
    try {
      ...
      const data = response?.data;
      if (data?.success) {
        setMessages(reset ? data?.data : (prevMessages) => [...prevMessages, ...data?.data]);
  ...
  const openConversation = (friend) => {
    ...
    // 🔥 Force fetching conversation immediately
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
            {errorMessage ? (
              ...
              filteredFriends.map((friend) => (
                <div
                  key={friend?._id}
                  className={`flex items-center gap-4 p-3 mb-3 rounded-xl hover:bg-white/10 cursor-pointer transition-colors ${
                    selectedFriend && selectedFriend._id === friend._id ? "bg-white/15" : ""
                  }`}
                ...
                {chatLoading ? (
                  ...
                  messages.map((msg) => (
                    <div
                      key={msg._id}
                      className={`flex ${
                        msg?.senderId !== selectedFriend?._id ? "justify-end" : "justify-start"
                      }`}
                    >
                      <div
